...
def recommend_ingredients(partial_recipes,
                          user_item_matrix,
                          k=10,
    ...
    if similarity_measure == "cosine":
        ...
        raise ValueError(
            "The similarity measure must be one of: 'cosine', 'asymmetric_cosine', 'jaccard', 'pmi'."
        )
    ...
    for elem in k:
        ...
        for i in range(np.shape(similarity_matrix)[0]):
            not_kNN = sim[i, ] < sim[i,
                                     np.argpartition(sim[i, ], -elem)[-elem]]
            sim[i, not_kNN] = 0

        # Calculate the ingredient scores.
        ingredient_scores = np.matmul(sim, partial_recipes.T) / np.sum(
            abs(sim), axis=1)[:, None]
        ...
        recommendations_idx = np.argsort(
            -ingredient_scores, axis=1).to_numpy()[:, :n_recommendations]

        # Convert recommendation indices to ingredient names.
        recommendations[elem] = np.array(
            user_item_matrix.columns)[recommendations_idx]

    return recommendations


def held_out_recommendation(user_item_matrix,
                            model_config=[10, "cosine", 10],
                            usePCA=False,
                            alpha=0.2):
    ...
    if usePCA == True:
        n = user_item_matrix.shape[1]  # number of ingredients
        pca = PCA(n_components=n)
        X_pca_T = pca.fit_transform(
            user_item_matrix.T)  # applying PCA to transformed version of data
    ...
    for index, row in user_item_matrix.iterrows():
        ...
        recipe = row.copy()
        ing = recipe[recipe == 1].sample(axis=0,
                                         random_state=1).index.values[0]
        recipe[ing] = 0

        # Get recommendations
        dict_ = recommend_ingredients(pd.DataFrame(recipe).T,
                                      X_curr,
                                      model_config[0],
                                      model_config[1],
                                      n_recommendations=model_config[2],
                                      alpha=alpha)
...
def metric_1(missing_ingredients, recommendations):
    ...
    matches = [
        1 for i in range(len(missing_ingredients))
        if missing_ingredients[i] in recommendations[i][:10]
    ]
...
def metric_2(missing_ingredients, recommendations):
    ...
    ranks = [
        np.where(missing_ingredients[i] == recommendations[i])[0][0]
        for i in range(len(missing_ingredients))
        if missing_ingredients[i] in recommendations[i]
    ]
...
def calculate_metrics(missing_ingredients, recommendations, k, sim):
    ...
    metrics.loc[0, "recall10"] = metric_1(missing_ingredients, recommendations)
    metrics.loc[0, "mean_rank"] = metric_2(missing_ingredients,
                                           recommendations)
    metrics.loc[0, "median_rank"] = metric_3(missing_ingredients,
                                             recommendations)
